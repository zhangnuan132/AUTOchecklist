...
class TestAddSub():
    ...
    def test_cover(self, init_driver):
        with allure.step('step 1、点击我的tab,我的-，进入封面测评页面'):
            driver = init_driver
            time.sleep(5)
            ...
            time.sleep(5)
        with allure.step('step 2、点击我的封面测评tab，进入页面'):
            click_methods_id_text(driver, list_str(josn_test['Modle_mine']['Mine_cover']['cover_id_text']),
                                  '封面测评')

            # #  方法2：适用于用于 不同模块相同id   根据id text 同时查询
            # cover_id_text = list_str(josn_test['Modle_mine']['Mine_cover']['cover_id_text'])  # 我的-封面测评
            # print("cover_id_text::::是str ", cover_id_text)
            # cover_id_text1 = driver.find_element(By.XPATH, cover_id_text).get_attribute('text') # 通过xpath得到可识别的text
            # print("cover_id_text1::::是str", type(cover_id_text1), cover_id_text1 )
            # if is_element_exist(str_list(cover_id_text1), page_source_cover):
            #     print("str_list(dynamic_id_text1)...是list", type(cover_id_text1), cover_id_text1)
            #     driver.find_element(By.XPATH, cover_id_text).click()  # 点击我的封面测评按钮
            #     assert "点击封面测评正常"
            # else:
            #     assert "点击封面测评失败"
        with allure.step('step 3、已经点击封面测评，判断点击后的页面是否正确'):
            exist_methods_subidtext(driver, list_str(josn_test['Modle_mine']['Mine_cover']['Mine_sub_id_text_cover']),'封面测评')
            driver.quit()
