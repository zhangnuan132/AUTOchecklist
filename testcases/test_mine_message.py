...
class TestAddSub():
    ...
    def test_cover(self, init_driver):
        with allure.step('step 1、点击我的tab，进入消息页面'):
            driver = init_driver
            time.sleep(5)
            ...
            time.sleep(5)
        with allure.step('step 2、点击我的-消息tab，进入页面'):
            click_methods_id_text(driver, list_str(josn_test['Modle_mine']['Mine_message']['message_id_text']),'消息')
        with allure.step('step 3、已经点击消息，判断点击后的页面是否正确'):
            exist_methods_subidtext(driver, list_str(josn_test['Modle_mine']['Mine_message']['Mine_sub_id_text_message']),'私信')
            exist_methods_subidtext(driver, list_str(josn_test['Modle_mine']['Mine_message']['Mine_sub_id_text_kefu']),'花椒客服')
            exist_methods_subidtext(driver, list_str(josn_test['Modle_mine']['Mine_message']['Mine_sub_id_text_call']),'打招呼')
            driver.quit()